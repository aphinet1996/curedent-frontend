...
import { useDoctorStore } from '~/stores/doctor' // เพิ่ม import
import { useAssistantStore } from '~/stores/assistant' // เพิ่ม import
...
const doctorStore = useDoctorStore() // เพิ่ม store
const assistantStore = useAssistantStore() // เพิ่ม store
...
const options = computed(() => patientStore.getPatientOptions)
...
const optionConfigs = {
  titlePrefix: {
    ...
    message: 'กรุณากรอกคำนำหน้าที่ต้องการเพิ่ม:',
    placeholder: 'เช่น นาย, นางสาว, ดร., ผศ.'
  },
  nationality: {
    title: 'เพิ่มสัญชาติใหม่',
    message: 'กรุณากรอกสัญชาติที่ต้องการเพิ่ม:',
    placeholder: 'เช่น ไทย, อเมริกัน, ญี่ปุ่น'
  },
  gender: {
    title: 'เพิ่มเพศใหม่',
    message: 'กรุณากรอกเพศที่ต้องการเพิ่ม:',
    placeholder: 'เช่น ชาย, หญิง, ไม่ระบุ'
  },
  patientType: {
    title: 'เพิ่มประเภทผู้ป่วยใหม่',
    message: 'กรุณากรอกประเภทผู้ป่วยที่ต้องการเพิ่ม:',
    placeholder: 'เช่น ผู้ป่วยทั่วไป, ผู้ป่วยพิเศษ'
  },
  bloodGroup: {
  ...
  occupation: {
    ...
    message: 'กรุณากรอกอาชีพที่ต้องการเพิ่ม:',
    placeholder: 'เช่น พนักงานบริษัท, ข้าราชการ, นักเรียน'
  },
  medicalRight: {
    title: 'เพิ่มสิทธิการรักษาใหม่',
    message: 'กรุณากรอกสิทธิการรักษาที่ต้องการเพิ่ม:',
    placeholder: 'เช่น ประกันสังคม, บัตรทอง, เบิกได้'
  },
  maritalStatus: {
    title: 'เพิ่มสถานภาพสมรสใหม่',
    message: 'กรุณากรอกสถานภาพสมรสที่ต้องการเพิ่ม:',
    placeholder: 'เช่น โสด, สมรส, หย่าร้าง, หม้าย'
  },
  referralSource: {
    title: 'เพิ่มแหล่งที่มาใหม่',
    message: 'กรุณากรอกแหล่งที่มาของผู้ป่วยที่ต้องการเพิ่ม:',
    placeholder: 'เช่น แนะนำจากเพื่อน, โฆษณา, อินเทอร์เน็ต'
  }
}
...
watch(() => props.show, (newVal) => {
  if (newVal) {
    ...
    fetchAllOptions()
    fetchBranches()
    fetchDoctorAndAssistantOptions() // เพิ่มการเรียก fetch options
  }
})
...
// เพิ่มฟังก์ชันสำหรับ fetch doctor และ assistant options
...
const handleAddNewOption = async (value: string) => {
  ...
  try {
    ...
    if (result) {
      alertStore.success(`เพิ่มตัวเลือก "${value}" สำเร็จ`, 'สำเร็จ')
    } else {
            ...
            <div>
              ...
              <div class="flex gap-2">
                <select v-model="form.titlePrefix" class="flex-1 border border-gray-300 rounded px-3 py-2 text-sm">
                  ...
                  <option v-for="prefix in options.titlePrefixes" :key="prefix" :value="prefix">
                    {{ prefix }}
                  </option>
                </select>
                <button 
            ...
            <div>
              ...
              <div class="flex gap-2">
                <select v-model="form.nationality" class="flex-1 border border-gray-300 rounded px-3 py-2 text-sm">
                  ...
                  <option v-for="nationality in options.nationalities" :key="nationality" :value="nationality">
                    {{ nationality }}
                  </option>
                </select>
                <button 
            ...
            <div>
              ...
              <div class="flex gap-2">
                <select v-model="form.patientType" class="flex-1 border border-gray-300 rounded px-3 py-2 text-sm">
                  ...
                  <option v-for="type in options.patientTypes" :key="type" :value="type">
                    {{ type }}
                  </option>
                </select>
                <button 
            ...
            <div>
              ...
              <div class="flex gap-2">
                <select v-model="form.bloodGroup" class="flex-1 border border-gray-300 rounded px-3 py-2 text-sm">
                  ...
                  <option v-for="blood in options.bloodGroups" :key="blood" :value="blood">
                    {{ blood }}
                  </option>
                </select>
                <button 
            ...
            <div>
              ...
              <div class="flex gap-2">
                <select v-model="form.occupation" class="flex-1 border border-gray-300 rounded px-3 py-2 text-sm">
                  ...
                  <option v-for="occupation in options.occupations" :key="occupation" :value="occupation">
                    {{ occupation }}
                  </option>
                </select>
                <button 
            ...
            <div>
              ...
              <div class="flex gap-2">
                <select v-model="form.medicalRights" class="flex-1 border border-gray-300 rounded px-3 py-2 text-sm">
                  ...
                  <option v-for="right in options.medicalRights" :key="right" :value="right">
                    {{ right }}
                  </option>
                </select>
                <button 
            ...
            <div>
              ...
              <div class="flex gap-2">
                <select v-model="form.maritalStatus" class="flex-1 border border-gray-300 rounded px-3 py-2 text-sm">
                  ...
                  <option v-for="status in options.maritalStatuses" :key="status" :value="status">
                    {{ status }}
                  </option>
                </select>
                <button 
            ...
            <div>
              ...
              <div class="flex gap-2">
                <select v-model="form.referralSource" class="flex-1 border border-gray-300 rounded px-3 py-2 text-sm">
                  ...
                  <option v-for="source in options.referralSources" :key="source" :value="source">
                    {{ source }}
                  </option>
                </select>
                <button 
