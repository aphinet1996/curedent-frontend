...
export const doctorService = {
  async getDoctors(params?: DoctorParams): Promise<ApiResponse<{ doctors: Doctor[] }>> {
    return await authenticatedRequest<ApiResponse<{ doctors: Doctor[] }>>('/doctors', {
      method: 'GET',
      params,
    });
  },
  ...
  async getDoctorById(id: string): Promise<ApiResponse<{ doctor: Doctor }>> {
    return await authenticatedRequest<ApiResponse<{ doctor: Doctor }>>(`/doctors/${id}`, {
      method: 'GET',
    });
  },

  async createDoctor(formData: FormData): Promise<ApiResponse<{ doctor: Doctor }>> {
    return await authenticatedRequest<ApiResponse<{ doctor: Doctor }>>('/doctors', {
      method: 'POST',
      body: formData,
    });
  },
  ...
  async deleteDoctor(id: string): Promise<boolean> {
    await authenticatedRequest<any>(`/doctors/${id}`, {
      method: 'DELETE',
    });
    return true;
  },
  ...
  async getDoctorOptions(): Promise<DoctorOption[]> {
    const response = await authenticatedRequest<DoctorOptionsResponse>('/doctors/option', {
      method: 'GET',
    });
